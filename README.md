...
3. **The price settles it.** The outcome is read from the pool's own `Swap` events. An outcome
   that cannot be read is declared `UNRESOLVABLE`, never guessed; above 20% of an epoch, that epoch
   pays nobody.
