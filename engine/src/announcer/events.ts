...
export async function collectEvents(db: Db, now: number): Promise<Array<{ event: AnnounceEvent; channels: Array<"telegram" | "x"> }>> {
  ...
  const qs = (await db.query<QRow>("SELECT id, epoch, deadline, tx_hash, token, symbol, json, outcome FROM questions WHERE status = 'OPEN' ORDER BY deadline, id")).rows;
  ...
  const eps = (await db.query<{ epoch: number; payload: string; published_at: Date | null }>(
    "SELECT epoch, payload, published_at FROM epochs WHERE state = 'PUBLISHED' ORDER BY epoch",
  )).rows;
  for (const e of eps) {
  ...
  const swaps = (await db.query<{ tx_hash: string; detail: string }>("SELECT tx_hash, detail FROM treasury_ops WHERE kind = 'SWAP' AND tx_hash IS NOT NULL ORDER BY id")).rows;
